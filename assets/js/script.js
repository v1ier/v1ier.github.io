...
let POEM_SRC = "content/poems/menjadi-lukamu.md";
...
async function resolvePoemSource() {
  try {
    ...
    const match = (slug && poems.find((entry) => entry.slug === slug)) || poems[0];

    if (match && match.file) {
      POEM_SRC = match.file;
    }
  } catch (error) {
    // Manifest unavailable: fall back to the default POEM_SRC / hardcoded poem below.
  }
}

function applyPoemMeta() {
  const title = poem.title || "Puisi";
  document.title = `${title} — Puisi vier`;
  if (loaderKicker) {
    loaderKicker.textContent = title;
  }
}

async function loadPoem() {
  setProgress(10, "Membuka puisi...");
  await resolvePoemSource();
  try {
